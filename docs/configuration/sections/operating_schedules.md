...
| **UsePowerTariff** | Set to True to use the defined power tariff rates for this schedule (see [PowerTariff](power_tariff.md)) |
...
| Price | The average eletricity price in c/kWh for energy during this window. Used to forecast costs when running on a schedule rather than Amber prices. Note: If UsePowerTariff is set to True for this schedule, then this price will only be used when there is no applicable power tariff rate for a certain time of day. |