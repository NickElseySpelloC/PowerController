...
def test_non_meter_run_is_split_across_midnight(monkeypatch: pytest.MonkeyPatch):
    ...
    times = {
        "init": dt.datetime(2026, 2, 23, 23, 50, tzinfo=local_tz),
        "start": dt.datetime(2026, 2, 23, 18, 56, tzinfo=local_tz),
        "last_tick": dt.datetime(2026, 2, 23, 23, 59, tzinfo=local_tz),
        "tick": dt.datetime(2026, 2, 24, 0, 1, tzinfo=local_tz),
    }
    clock = {"now": times["init"]}
    ...
    def make_status(*, is_on: bool = True) -> OutputStatusData:
        return OutputStatusData(
            meter_reading=0,
            power_draw=0.0,
            is_on=is_on,
    ...
    rh.start_run(SystemState.AUTO, StateReasonOn.ACTIVE_RUN_PLAN, make_status(), start_time=times["start"])
    rh.last_tick = times["last_tick"]

    clock["now"] = times["tick"]
    have_rolled = rh.tick(make_status())
    assert have_rolled is True
    ...
    assert yesterday["Date"] == dt.date(2026, 2, 23)
    assert today["Date"] == dt.date(2026, 2, 24)

    # Yesterday's open run should be closed at day end.
    yesterday_run = yesterday["DeviceRuns"][-1]
    assert yesterday_run["ReasonStopped"] == StateReasonOff.DAY_END
    assert yesterday_run["EndTime"] == dt.datetime(2026, 2, 23, 23, 59, 59, tzinfo=local_tz)

    # Today's run should be created at midnight with DAY_START.
    assert today["DeviceRuns"], "Expected a new run to be created for the new day"
    today_run = today["DeviceRuns"][0]
    assert today_run["ReasonStarted"] == StateReasonOn.DAY_START
    assert today_run["StartTime"] == dt.datetime(2026, 2, 24, 0, 0, 0, tzinfo=local_tz)
    assert today_run["EndTime"] is None
